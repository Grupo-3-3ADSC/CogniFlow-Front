...
export function Relatorios() {
  ...
  async function baixarExcelEntradas(ordens) {
    ...
    // CRIA O MAPA DO IPI AQUI DENTRO (garante que o estoque já carregou)
// NOVO - 100% FUNCIONAL
const estoqueIpiMap = {};
estoque.forEach(item => {
  const id = Number(item.id);
  const ipiValor = item.ipi !== undefined && item.ipi !== null 
    ? Number(item.ipi) 
    : 0;
  estoqueIpiMap[id] = ipiValor;
});

console.log("MAPA DE IPI CORRIGIDO:", estoqueIpiMap); // ← agora vai aparecer {1: 5, 2: 0, 3: 3, ...}
...
// ADICIONE ISSO LOGO APÓS CRIAR O MAPA
console.log("=== DEBUG IPI ===");
console.log("estoque carregado:", estoque);
console.log("estoqueIpiMap criado:", estoqueIpiMap);
console.log("primeira ordem (exemplo):", ordens[0]);
console.log("IPI que deveria aparecer:", estoqueIpiMap[ordens[0]?.estoque_id]);
console.log("==================");
    // === Linhas da tabela com zebra ===
   ordens.forEach((o, index) => {
  const ipi = estoqueIpiMap[o.material_id || o.estoqueId || o.id_material || o.estoque_id] ?? estoqueIpiMap[Number(o.material_id || o.estoqueId || o.id_material || o.estoque_id)] ?? 0;
  const precoSemIpi = (o.valorUnitario || 0) * (o.quantidade || 0);
  ...
  const row = sheet.addRow([
    ...
    o.quantidade || 0,
    o.valorUnitario || 0,
    precoSemIpi,     // ← agora usa a variável correta
    ipi,
    precoComIpi
  ]);
